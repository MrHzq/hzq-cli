...
module.exports = async (_, options = {}) => {
  ...
  const initVar = (answers) => {
    ...
    needLs = answers.needLs;
    configByProject = answers.configByProject;
  ...
  const runMain = async () => {
    if (checkFileExist(newFilePath)) {
      const fileList = reReaddirSync(newFilePath);
      if (fileList.length) return `${cmd} 文件已存在`;
      else removeSync(newFilePath);
    }
    ...
    async prompt(_config, ...args) {
      ...
      answers = await prompt([
        ...
        },
        {
          type: "list",
          name: "needMuteConfig",
          message: "配置是否存在一个 key 有多个的情况:",
          choices: tfList(),
          when: (answer) => answer.needConfig,
        },
        {
          type: "list",
          name: "configByProject",
          message: "配置是否以项目维度有多个的情况:",
          choices: tfList(),
          when: (answer) => answer.needConfig && !answer.needMuteConfig,
        },
        {
          type: "list",
          name: "needInquirer",
        ...
        },
        {
          ...
          message: "是否需要 ls:",
          choices: tfList(),
          when: (answer) => answer.needConfig || answer.needInquirer,
        },
      ]);
